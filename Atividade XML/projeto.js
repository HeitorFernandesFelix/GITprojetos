...
function parserXMLUOL(xml){
  ...
  $publicacoes = $xml.find("item");
  
  $publicacoes.each(function(){
    titulo = $(this).find('title').text();
    descricao = $(this).find('description').text();
    publicacao = $(this).find('pubDate').text();
    dadosLista += criaElementoLista([titulo, descricao, publicacao]);
  });
  document.getElementById("projeto_lista").innerHTML = dadosLista;
}
